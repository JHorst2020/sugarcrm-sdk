import { SugarFilterType, FilterReturnType, FilterAndObject, FilterObject, OrFilterObject } from "../types"
 ...
 */
export const createSugarFilter = (filter: SugarFilterType): FilterReturnType => {
    const params = Object.keys(filter);
    const filterArr: FilterObject[] = [];

    for (let param of params) {
        const value = filter[param];
        if (Array.isArray(value)) {
            if (value.length > 0) {
                const $orFilter: OrFilterObject[] = value.map((ele) => {
                    return { [param]: ele } as OrFilterObject;
                });
                filterArr.push({ $or: $orFilter });
            }
        } else if (value !== undefined) {
            filterArr.push({ [param]: value } as OrFilterObject);
        }
    }
